...
2. right click on project folder and select open integrated termina;
    ![alt text](./image.png)
...
5. update type as `type:module` in package.json
    ![alt text](./image.png)
6. type in termial `npm i nodemon -D` to install nodemon, which restarts server while file changes. -D flag indicates install as dev dependency
...
9. update package.json to run the project, update script property as below:
  ...
  ````
10. now you can start the server by typing `npm run dev` in the terminal o fthe project folder